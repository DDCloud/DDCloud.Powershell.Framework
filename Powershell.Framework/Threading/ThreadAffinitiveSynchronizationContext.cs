...
namespace DDCloud.Powershell.Framework.Threading
{
	...
	/// </summary>
	/// <remarks>
	///		Powershell only allows a single thread to call Cmdlet methods (the thred on which the Cmdlet was started).
	///		For Cmdlets that need to use <c>async</c>/<c>await</c>, this synchronisation context enables natural-looking code to to still reliably call methods such as WriteWarning or WriteObject.
	/// 
